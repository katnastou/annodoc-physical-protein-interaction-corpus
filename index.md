...
We quantified the Inter-Annotator-Agreement (IAA) by calculating Cohen’s kappa for the pair of annotators. Cohen’s kappa is defined as κ = (Po – Pe)/(1-Pe). Po refers to the observed probability of agreement between two annotators, whereas Pe is the expected probability of agreement by random chance. For the first round of IAA Cohen's kappa was κ = 0.90. 
...
After updating the guidelines the annotators were provided with an additional set of 20 documents from the BioNLP ST 2009 development set and their IAA was calculated again and was found to be κ = 0.93. 
...
Since, in the previous three rounds of IAA, κ was always found to be very high, we decided to have one final round if its value was above 0.9. This time 20 documents from the 400 abstracts extracted from resources enriched in positive relationships were selected and annotated by both annotators. Their agreement was still very high (κ = 0.91) and that concluded the IAA process.
